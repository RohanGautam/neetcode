class Solution:
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        # should be a set for quick lookup
        # alternativey set to water once you visit so it just wont be picked up as a valid neighbour next time.
        seen = set()
        count=0
            ...
            for j in range(cols):
                if grid[i][j]=='1' and (i,j) not in seen:
                    ...
                    while len(q)!=0:
                        ...
                        # this is O(N)
                        cur = q.pop() # not pop(0), use like stack, dfs/bfs doesnt matter
                        seen.add(cur)
